...
<form method="post" name="form_adv_search" id="form_adv_search">
	<div class="adv_search panel panel-default display-none" id="adv_search">
		...
		<div class="panel-body">
			...
			<div class="form-group col-sm-6">
				<label class="col-sm-4 control-label" for="li_content">产品特点：</label>
				<div class="col-sm-8">
...
<?php if($auth['write']): ?><div class="space-8"></div>
...
<div class="">
	<?php if(empty($list)): ?><ul>
		...
		<?php else: ?>
			<ul>
				<?php if(is_array($list)): foreach($list as $key=>$vo): ?><li class="col-sm-3">
						<div class="product-wrap">
							<?php echo W('Product',array('add_file'=>$vo['add_file'],'mode'=>'show','id'=>$vo['id']));?>
							<div class="row">
								<div class="col-sm-12"><?php echo ($vo["product_no"]); ?></div>
							</div>
							<div class="row">
								<div class="col-sm-12 "><a  href="<?php echo U('read','id='.$vo['id']);?>"><?php echo ($vo["name"]); ?></a></div>
							</div>
						</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		set_return_url();
		$(".p-scroll img").mouseenter(function(){
			$img=$(this).attr("src");
			$(this).parents(".product-wrap").find(".p-img img").attr("src",$img);
		});

		$(".p-scroll img").click(function(){
			$img=$(this).attr("src");
			$(this).parents(".product-wrap").find(".p-img img").attr("src",$img);
		});
	}); 
