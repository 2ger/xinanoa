...
<?php echo W('PageHeader',array('name'=>'项目搜索','search'=>'M'));?>
		...
		<?php if(empty($list)): ?><li class="no-data">
			...
			<form method="post" action="" id="form_data">
				<?php if(is_array($list)): foreach($list as $key=>$vo): ?><li class="tbody">
							<span class="col-8 text-center"><?php echo ($vo["xm_no"]); ?></span>
							<span class="col-8 text-center"><?php echo ($vo["level_name"]); ?></span>
							<div class="pull-right">
								<span class="col-9  text-center"><?php echo ($vo["status_name"]); ?></span>
								<span class="col-9  text-center"><?php echo (todate($vo["update_time"],'Y-m-d')); ?></span>
							</div>
						<span class="auto autocut"> <a href="<?php echo U('read','id='.$vo['id']);?>"><?php echo ($vo["name"]); ?>
							<?php if((strlen($vo["name"])) == "0"): ?>无标题<?php endif; ?> </a> </span>
