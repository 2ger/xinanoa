...
<?php echo W('PageHeader',array('name'=>'消费记录','search'=>'N'));?>
	<div class="operate panel panel-default">
		<div class="panel-body">
			...
			<div class="pull-right">
				<a class="btn btn-sm btn-primary" onclick="add_sales(<?php echo ($vip_id); ?>)">新建</a>
			</div>
		</div>
	</div>
<div class="ul_table message-container">
	<ul>
		<li class="thead">			
			<span class="col-10 text-center">款号</span>
			<span class="col-10 text-center">日期</span>
			<span class="col-10 text-right">销售量</span>
			<span class="col-10 text-right">销售金额</span>
			<span class="col-10 text-right">购物积分</span>
		...
		<?php if(empty($list)): ?><li class="no-data">
			...
			<form method="post" action="" name="form_data" id="form_data">
				<?php if(is_array($list)): foreach($list as $key=>$vo): ?><li class="tbody data_item">
						<span class="col-10 text-center"><?php echo ($vo["product"]); ?></span>
						<span class="col-10 text-center"><?php echo ($vo["sales_date"]); ?></span>
						<span class="col-10 text-right data data_1"><?php echo ($vo["qty"]); ?></span>
						<span class="col-10 text-right data data_2"><?php echo (($vo["amount"])?($vo["amount"]):0); ?></span>
						<span class="col-10 text-right data data_3"><?php echo (($vo["point"])?($vo["point"]):0); ?></span>
						<span class="col-4 text-center data "><a class="link" onclick="edit_sales(<?php echo ($vo["id"]); ?>)">修改</a></span>
						<span class="col-4 text-center data "><a class="link-danger" onclick="del_sales(<?php echo ($vo["id"]); ?>);">删除</a></span>						
					</li><?php endforeach; endif; ?>
				<li class="tbody data_total">
					<span class="col-20 text-center">合计</span>
...
<script type="text/javascript">
	...
	function edit_sales($id){
		window.open(fix_url("<?php echo U('edit_sales');?>?id="+$id), "_self");
		return false;
	}

	function del_sales($id){
		ui_confirm('确定要删除吗?',function(){
			window.open(fix_url("<?php echo U('del_sales');?>?id="+$id), "_self");
		});
	}
