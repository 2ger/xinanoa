...
<?php echo W('PageHeader',array('name'=>'站内消息','search'=>'M'));?>
...
<div class="ul_table">
	<ul>
		<?php if(empty($list)): ?><li class="no-data">
			...
			<?php else: ?>
			<?php if(is_array($list)): foreach($list as $key=>$vo): ?><div class="dialogs">
					<div class="itemdiv dialogdiv">
						<div class="user">
						<?php if(($vo["sender_id"]) == $owner_id): ?><img  src="<?php echo get_save_url();?>emp_pic/<?php echo ($vo["receiver_id"]); ?>.jpeg" /><?php endif; ?>
						<?php if(($vo["receiver_id"]) == $owner_id): ?><img  src="<?php echo get_save_url();?>emp_pic/<?php echo ($vo["sender_id"]); ?>.jpeg" /><?php endif; ?>
						</div>
						<div class="body">
							<div class="time">
								<i class="fa fa-time"></i>
								<span><?php echo (todate($vo["create_time"],'Y-m-d h:m:d')); ?></span>
							</div>
							<div class="name">
								<?php if(($vo["sender_id"]) == $owner_id): ?>发给 <?php echo ($vo["receiver_name"]); endif; ?>
								<?php if(($vo["receiver_id"]) == $owner_id): echo ($vo["sender_name"]); endif; ?>
							</div>
							<div class="row">
								<div class="text-left col-sm-9 col-xs-12">
									<?php if(($vo["sender_id"]) == $owner_id): ?><a href="<?php echo U('read','reply_id='.$vo['receiver_id']);?>"><?php echo ($vo["content"]); ?></a><?php endif; ?>
									<?php if(($vo["receiver_id"]) == $owner_id): if(($vo["is_read"]) == "0"): ?><b><?php endif; ?>
										<a href="<?php echo U('read','reply_id='.$vo['sender_id']);?>"><?php echo ($vo["content"]); ?></a>
										<?php if(($vo["is_read"]) == "0"): ?></b><?php endif; endif; ?>
									<span class="badge"><?php echo ($vo["count"]); ?></span>
								</div>
								<div class="text-right pull-right col-sm-3 col-xs-12">
									<?php if(($vo["sender_id"]) == $user_id): ?><a class="btn-sm btn-primary" href="<?php echo U('read','reply_id='.$vo['receiver_id']);?>"><i class="fa fa-only fa-share-alt"></i>回复</a>
										<a class="btn-sm btn-danger" href="#" onclick="del_dialogue(<?php echo ($vo["receiver_id"]); ?>)">
										<i class="fa fa-only fa-remove" ></i>删除</a><?php endif; ?>
									<?php if(($vo["receiver_id"]) == $user_id): ?><a class="btn-sm btn-primary" href="<?php echo U('read','reply_id='.$vo['sender_id']);?>"><i class="fa fa-only fa-share-alt"></i>回复</a>
										<a class="btn-sm btn-danger" href="#" onclick="del_dialogue(<?php echo ($vo["sender_id"]); ?>)">
										<i class="fa fa-only fa-remove" ></i>删除</a><?php endif; ?>									
								</div>
							</div>
						</div>
...
<script type="text/javascript">
	function del_all(){
		ui_confirm('确定要删除全部消息吗?',function(){
			sendAjax("<?php echo U('del','type=all');?>",'', function(data) {
				if (data.status) {
	...
	function del_dialogue($reply_id){
