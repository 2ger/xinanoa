...
<?php echo W('PageHeader',array('name'=>字段管理,'search'=>'N'));?>
<div class="operate panel panel-default">
	<div class="panel-body">
		<div class="pull-left">
			...
			<div class="form-group">
				...
				<div class="col-sm-8">
					<select name="control_type" id="control_type" check="require" msg="请选择控件类型">
						...
						<option value="datem">日期 (年-月)
						<option value="dated">日期 (年-月-日)
						<option value="date">日期 (年-月-日 时:分)
						<option value="select">列表
						<option value="radio">单选
						<option value="checkbox">多选				
						<option value="textarea">多行文本
						<option value="simple">简易编辑器
						<option value="editor">编辑器
						<option value="file">上传附件
						<option value="add">新增-出差
						<option value="help">帮助
						<option value="hr">分隔符
					</select>
				</div>
			</div>
			...
			<div class="form-group">
				...
				<div class="col-sm-8">
					<select name="validate" id="validate">
						<option value="">请选择
						<option value="require">必选
						<option value="email">邮件
						<option value="number">数字
					</select>
				</div>
			</div>
			<div class="form-group">
	...
	$(document).ready(function(){
		$(".sub_left_menu .tree_menu  a").click(function() {
			...
			$(this).addClass("active");
				sendAjax("<?php echo U('flow_type/get_field');?>","id=" + $(this).attr("node"), function(data) {
				showdata(data);
			});
			return false;
		});
