...
<?php echo W('PageHeader',array('name'=>'业务权限分配','search'=>'N'));?>
	...
	<form id="form_data" method="post" action="">
				...
				<tbody>
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr id="<?php echo ($data["id"]); ?>">
							<td class="col-3"><label class="inline pull-left">
								<input class="ace" type="checkbox" name="role_id[]" value="<?php echo ($data["id"]); ?>" />
		...
		<div class="col-sm-8 sub_content">
			...
			<table class="table table-bordered">
				...
				<tbody>
					<?php if(is_array($duty_list)): $i = 0; $__LIST__ = $duty_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr id="<?php echo ($data["id"]); ?>">
							<td class="col-3"><label class="inline pull-left ">
								<input class="ace" type="checkbox" id="duty_<?php echo ($data["id"]); ?>" name="duty_list[]" value="<?php echo ($data["id"]); ?>"/>
								<span class="lbl"></span></label></td>
							<td class="col-10"><?php echo ($data["duty_no"]); ?></td>
							<td class="col-10"><?php echo ($data["name"]); ?></td>
...
<script type="text/javascript">
	...
	function save() {
		sendForm("form_data", "<?php echo U('set_duty');?>");
	};
	...
	$(document).ready(function() {
		...
		$(".sub_left_menu  tr").click(function() {
			...
			sendAjax("<?php echo U('get_duty_list');?>", "role_id=" + $(this).attr("id"), function(data) {
				showdata(data);
			});
		});
	}); 
