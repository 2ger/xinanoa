...
<?php echo W('PageHeader',array('name'=>'论坛','search'=>'N'));?>
<div class="ul_table">
	<ul>
		<li class="thead">
			<div class="pull-right">
				<span class="col-16 text-center"> 管理者 </span>
				<span class="col-9  text-center"> 主题/帖子</span>			
				<span class="col-9  text-center"> 今日</span>
			</div>
			<div class="auto">
				 版块
			</div>
	...
	<?php if(count($forum_list) == 0): ?><ul>
		...
		<?php else: ?>
		<ul>
			<?php if(is_array($forum_list)): foreach($forum_list as $key=>$vo): ?><li class="tbody" >
					<div class="pull-right">
						<span class="col-16 text-center autocut"><?php echo (show_contact($vo["admin"])); ?>&nbsp;</span>
						<span class="col-9  text-center"> <?php echo (($forum_info[$vo['id']]['forum'])?($forum_info[$vo['id']]['forum']):0); ?>/<?php echo (($forum_info[$vo['id']]['post'])?($forum_info[$vo['id']]['post']):0); ?></span>
						<span class="col-9  text-center"> <?php echo (($today_count[$vo['id']]['today_count'])?($today_count[$vo['id']]['today_count']):0); ?></span>
					</div>
					<div class="auto">
						<a href="<?php echo U('folder','fid='.$vo['id']);?>" ><?php echo ($vo["name"]); ?>
...
<script type="text/javascript">
	...
	function del() {
		ui_confirm('确定要删除吗?',function(){
			sendAjax("<?php echo U('del');?>", 'id=' + $("#forum_id").val(), function(data) {
				if (data.status) {
