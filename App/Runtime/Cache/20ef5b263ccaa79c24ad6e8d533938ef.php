...
<div class="ul_table">
	...
	<?php if(empty($list)): ?><ul>
		...
		<form method="post" action="" name="form_data" id="form_data">
			<ul>
				<?php if(is_array($list)): foreach($list as $id=>$vo): ?><li class="tbody">
						<div class="pull-left">
							<span class="col-5 "><?php echo ($id+1); ?><!-- <?php echo ($vo["doc_no"]); ?> --></span>
							<span class="col-7"> <?php echo ($vo["user_name"]); ?></span>
						</div>
						<div class="pull-right">
							<span class="col-12   "> <?php echo (todate($vo["create_time"],'y/m/d h:i')); ?> </span>
							<span class="col-25   "><?php echo ($vo["confirm_name"]); ?></span>
							<span class="col-12   ">
								<?php echo (show_step($vo["step"])); ?>
							    <?php if(($repeal) == "1"): ?><a href="#" class="btn btn-danger btn-xs">撤销</a><?php endif; ?>
								<?php if(($repeat) == "1"): ?><a href="#" class="btn btn-success btn-xs">重办</a><?php endif; ?>
							
																			
							</span>
						</div>
						<div class="autocut auto">
						  <a href="<?php echo U('read','id='.$vo['id'].'&fid='.$folder);?>">ss<?php echo ($vo["name"]); ?>
							<?php if(in_array(($folder), explode(',',"darft"))): ?><a href="<?php echo U('edit','id='.$vo['id'].'&fid='.$folder);?>"><?php echo ($vo["type_name"]); ?>
								<?php else: ?>
								<a href="<?php echo U('read','id='.$vo['id'].'&fid='.$folder);?>"><?php echo ($vo["type_name"]); endif; ?>
