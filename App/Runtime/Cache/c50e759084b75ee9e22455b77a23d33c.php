		...
		<form method="post" action="" name="form_data" id="form_data">
			<ul>
				<?php if(is_array($list)): foreach($list as $key=>$vo): ?><li class="tbody">
						<div class="pull-left">
							<span class="col-16 "> <?php echo (todate($vo["create_time"],'Y-m-d')); ?></span>
							<span class="col-16  "> <?php echo ($vo["user_name"]); ?></span>
						</div>
						<div class="pull-right">
                            
                            <div class="progress">
                              <div class="progress-bar" role="progressbar" aria-valuenow="<?php echo (progress($vo["id"])); ?>" aria-valuemin="0" aria-valuemax="100" style="width: <?php echo (progress($vo["id"])); ?>%;">
                                <?php echo (progress($vo["id"])); ?>%
                              </div>
                            </div>
                            
							<span class="col-9   "><?php echo (dai_step($vo["id"])); ?> </span>
							<span class="col-6   "><?php echo (nownman($vo["id"])); ?> </span>
							<span class="col-6   "><?php echo (show_step($vo["step"])); ?></span>
						</div>
						<div class="autocut auto">
							<?php if(in_array(($folder), explode(',',"darft"))): ?><a href="<?php echo U('edit','id='.$vo['id'].'&fid='.$folder);?>"><?php echo (get_customer_name($vo["id"])); ?>
								<?php else: ?>
								<a href="<?php echo U('read','id='.$vo['id'].'&fid='.$folder);?>"><?php echo (get_customer_name($vo["id"])); endif; ?>
