...
<!--  邮件分类 -->
<?php echo W('PageHeader',array('name'=>'邮件分类','search'=>'N'));?>
			...
			<form method='post' id="form_data">
				<?php if(is_array($list)): foreach($list as $key=>$vo): ?><li class="tbody">
						<span class="pull-right col-12 text-center">
							...
							<a  onclick="del(<?php echo ($vo["id"]); ?>);return false;">删除</a></span>
						<span class="pull-right col-3 text-center"><?php echo (status($vo["is_del"])); ?> </span>
						<span class="auto"> 如果：
							<?php echo (mail_org_string($vo)); ?> </span>
...
<script type="text/javascript">
	...
	function del(id) {
		ui_confirm('确定要删除吗?',function(){
			window.open(fix_url("<?php echo U('del');?>?id=" + id), "_self");
		});
	};
